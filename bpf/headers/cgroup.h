                                          ...
                                          struct cgroup_info *cg_info)
{
    ...
    void *info = bpf_map_lookup_elem(&fsm_cgr_fib, &cgroup_id);
    if (!info) {
        ...
        if (s) {
            ...
            // get ip addresses of current pod/ns.
            __u32 *ip =
                (__u32 *)bpf_map_lookup_elem(&fsm_mark_fib, &curr_ip_mark);
            if (!ip) {
...
#endif
        if (bpf_map_update_elem(&fsm_cgr_fib, &cgroup_id, &_default, BPF_ANY)) {
            printk("update fsm_cgr_fib of cgroup(%ld) error", cgroup_id);
            return 0;
        }
        *cg_info = _default;
    } else {
