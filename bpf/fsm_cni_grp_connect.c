...
static inline int fsm_cni_tcp_connect4(struct bpf_sock_addr *ctx)
{
    ...
    debugf("fsm_cni_tcp_connect4 uid: %d cur pod ip: %pI4 dst ip: %pI4", uid,
           &curr_pod_ip, &dst_ip);
    if (uid != SIDECAR_USER_ID) {
        if ((dst_ip & 0xff) == 0x7f) {
            debugf("fsm_cni_tcp_connect4 [App->Local]: bypass");
            // app call local, bypass.
            return 1;
        }
        __u64 cookie = bpf_get_socket_cookie_addr(ctx);
        // app call other app
        debugf("fsm_cni_tcp_connect4 [App->App]: dst ip: %pI4 dst port: %d",
        ...
        origin.flags = 1;
        if (bpf_map_update_elem(&fsm_cki_fib, &cookie, &origin, BPF_ANY)) {
            debugf("fsm_cni_tcp_connect4 write fsm_cki_fib failed");
            return 0;
        }
        if (curr_pod_ip) {
            struct pod_config *pod =
                bpf_map_lookup_elem(&fsm_pod_fib, _curr_pod_ip);
            if (pod) {
                int exclude = 0;
                IS_EXCLUDE_PORT(pod->exclude_out_ports, ctx->user_port,
                                &exclude);
                if (exclude) {
                    debugf("fsm_cni_tcp_connect4 ignored dst port by "
                ...
                debugf("fsm_cni_tcp_connect4 exclude ipranges: %x, exclude: %d",
                       pod->exclude_out_ranges[0].net, exclude);
                if (exclude) {
                    debugf("fsm_cni_tcp_connect4 ignored dest ranges by "
                ...
                IS_INCLUDE_PORT(pod->include_out_ports, ctx->user_port,
                                &include);
                if (!include) {
                    debugf("fsm_cni_tcp_connect4 dest port %d not in "
                ...
                if (!include) {
                    debugf("fsm_cni_tcp_connect4 dest %pI4 not in pod(%pI4)'s "
                           "include_out_ranges, ignored.",
                           &dst_ip, &curr_pod_ip);
                    return 1;
                }
            } else {
                debugf("fsm_cni_tcp_connect4 current pod ip found(%pI4), but "
                       "can not find pod_info from fsm_pod_fib",
                       &curr_pod_ip);
            }
            ...
            if (bpf_bind(ctx, &addr, sizeof(struct sockaddr_in))) {
                debugf("fsm_cni_tcp_connect4 bind %pI4 error", &curr_pod_ip);
            }
            ctx->user_ip4 = localhost;
        } else {
            debugf("fsm_cni_tcp_connect4 curr_pod_ip false");
        ...
        debugf("fsm_cni_tcp_connect4 [App->Sidecar]: rewrite dst ip: %pI4, "
               "redirect dst port: %d",
        ...
        struct pod_config *pod = bpf_map_lookup_elem(&fsm_pod_fib, _dst_ip);
        if (!pod) {
            // bypass
            debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: dst ip: %pI4 dst "
                   "port: %d bypass",
                   &dst_ip, bpf_htons(ctx->user_port));
            return 1;
        }
        ...
        debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: uid: %d", uid);
        debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: cur pod ip: %pI4",
               &curr_pod_ip);
        debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: dst pod ip: %pI4 dst "
               "port: %d",
               &dst_ip, bpf_htons(ctx->user_port));
        ...
        if (curr_pod_ip) {
            if (curr_pod_ip != dst_ip) {
                ...
                IS_EXCLUDE_PORT(pod->exclude_in_ports, ctx->user_port,
                                &exclude);
                if (exclude) {
                    debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: ignored "
                ...
                debugf("fsm_cni_tcp_connect4 [Sidecar->Others{Sidecar}]: "
                ...
                debugf("fsm_cni_tcp_connect4 [Sidecar->Others{Self}]");
            }
            origin.flags |= 1;
        } else {
            ...
            void *curr_ip = bpf_map_lookup_elem(&fsm_proc_fib, &pid);
            debugf("fsm_cni_tcp_connect4 [Sidecar->Others]: pid: %d", pid);
            if (curr_ip) {
        ...
        __u64 cookie = bpf_get_socket_cookie_addr(ctx);
        if (bpf_map_update_elem(&fsm_cki_fib, &cookie, &origin, BPF_NOEXIST)) {
            printk("fsm_cni_tcp_connect4 update cookie origin failed");
            return 0;
        }
    }
...
__section("cgroup/connect4") int fsm_cni_group_connect4(
    struct bpf_sock_addr *ctx)
{
    switch (ctx->protocol) {
    case IPPROTO_TCP:
        return fsm_cni_tcp_connect4(ctx);
    default:
        return 1;
    }
}
