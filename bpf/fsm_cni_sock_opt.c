...
__section("cgroup/getsockopt") int fsm_cni_sock_opt(struct bpf_sockopt *ctx)
{
    ...
    // sidecar will call getsockopt with SO_ORIGINAL_DST, we should rewrite it to
    // return original dst info.
    if (ctx->optname != SO_ORIGINAL_DST) {
        return 1;
    }
    debugf("fsm_cni_sock_opt ctx->optname:SO_ORIGINAL_DST");
    struct pair p;
    memset(&p, 0, sizeof(p));
    ...
    case 2: // ipv4
        ...
        debugf("fsm_cni_sock_opt src ip4: %pI4 src port: %d", &src_ip4,
               bpf_htons(p.sport));
        debugf("fsm_cni_sock_opt dst ip4: %pI4 dst port: %d", &dst_ip4,
               bpf_htons(p.dport));
#endif
        origin = bpf_map_lookup_elem(&fsm_nat_fib, &p);
        if (origin) {
            ...
            *(struct sockaddr_in *)ctx->optval = sa;
            ...
            debugf("fsm_cni_sock_opt origin dst ip4: %pI4 origin dst port: %d",
                   &origin_ip4, bpf_htons(origin->port));
#endif
        } else {
            debugf("fsm_cni_sock_opt fsm_nat_fib:NOT FOUND");
        }
        break;
    }
    return 1;
