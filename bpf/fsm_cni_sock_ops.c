...
static inline int fsm_cni_sockops_ipv4(struct bpf_sock_ops *skops)
{
    struct pair p;
    memset(&p, 0, sizeof(p));
    ...
    struct origin_info *dst = bpf_map_lookup_elem(&fsm_cki_fib, &cookie);
    if (dst) {
        ...
        if (!(dd.flags & 1)) {
            ...
            if (skops->local_ip4 == sidecar_ip ||
                ...
                debugf("fsm_cni_sockops_ipv4 [Sidecar->Local] detected process "
                       "%d's ip is %pI4",
                       pid, &ip);
                bpf_map_update_elem(&fsm_proc_fib, &pid, &ip, BPF_ANY);
                ...
                bpf_map_update_elem(&fsm_proc_fib, &pid, &ip, BPF_ANY);
                debugf("fsm_cni_sockops_ipv4 [Sidecar->Sidecar] detected "
        ...
        debugf("fsm_cni_sockops_ipv4 [established] remote_ip4: %pI4 -> "
               "local_ip4: %pI4",
               &remote_ip4, &local_ip4);
        debugf("fsm_cni_sockops_ipv4 [established] remote_port: %d -> "
               "local_port: %d",
               bpf_htons(p.dport), skops->local_port);
#endif
        // get_sockopts can read pid and cookie,
        // we should write a new map named fsm_nat_fib
        bpf_map_update_elem(&fsm_nat_fib, &p, &dd, BPF_ANY);
        bpf_sock_hash_update(skops, &fsm_sock_fib, &p, BPF_NOEXIST);
...
__section("sockops") int fsm_cni_sock_ops(struct bpf_sock_ops *skops)
{
    switch (skops->op) {
    ...
    case BPF_SOCK_OPS_ACTIVE_ESTABLISHED_CB:
        ...
        case 2:
            ...
            return fsm_cni_sockops_ipv4(skops);
        }
        return 0;
    }
    return 0;
