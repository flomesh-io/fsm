...
__section("classifier_ingress") int fsm_cni_tc_dnat(struct __sk_buff *skb)
{
    ...
    if (tcph->syn && !tcph->ack) {
        ...
        if (tcph->dest == in_port) {
            ...
            debugf("fsm_cni_tc_nat [ingress]: already dnat");
            return TC_ACT_OK;
        }
        // ingress without fsm_cni_grp_connect
        struct pod_config *pod = bpf_map_lookup_elem(&fsm_pod_fib, dst_ip);
        if (!pod) {
            // dest ip is not on this node or not injected sidecar.
            debugf("fsm_cni_tc_nat [ingress]: pod not found, bypassed");
            return TC_ACT_OK;
        }
        ...
        if (exclude) {
            debugf("fsm_cni_tc_nat [ingress]: ignored dest port by "
        ...
        bpf_map_update_elem(&fsm_nat_fib, &p, &origin, BPF_NOEXIST);
        ...
        debugf("fsm_cni_tc_nat [ingress]: first dnat");
    } else {
        // request
        struct pair p;
        memset(&p, 0, sizeof(p));
        ...
        p.sport = tcph->source;
        p.dport = in_port;
        struct origin_info *origin = bpf_map_lookup_elem(&fsm_nat_fib, &p);
        if (!origin) {
            return TC_ACT_OK;
        }
        if (!(origin->flags & TC_ORIGIN_FLAG)) {
            // not tc origin
            debugf("fsm_cni_tc_nat [ingress]: no tc origin flag");
            return TC_ACT_OK;
        }
        __u16 dst_port = tcph->dest;
        bpf_l4_csum_replace(skb, csum_off, dst_port, in_port, sizeof(dst_port));
        bpf_skb_store_bytes(skb, dport_off, &in_port, sizeof(in_port), 0);
        debugf("fsm_cni_tc_nat [ingress]: dnat");
    }
    return TC_ACT_OK;
}
...
__section("classifier_egress") int fsm_cni_tc_snat(struct __sk_buff *skb)
{
    ...
    if (tcph->source != in_port) {
        // debugf("fsm_cni_tc_nat[egress]: no need to rewrite src port,
        // bypassed");
        return TC_ACT_OK;
    }
    struct pair p;
    ...
    if (!origin) {
        // not exists
        debugf("fsm_cni_tc_nat [egress]: resp origin not found");
        return TC_ACT_OK;
    }
    if (!(origin->flags & TC_ORIGIN_FLAG)) {
        // not tc origin
        printk("fsm_cni_tc_nat [egress]: resp origin flags %x error",
               origin->flags);
        return TC_ACT_OK;
    }
    if (tcph->fin && tcph->ack) {
        // todo delete key
        debugf("fsm_cni_tc_nat [egress]: original deleted");
        bpf_map_delete_elem(&fsm_nat_fib, &p);
    }
    ...
    debugf("fsm_cni_tc_nat [egress]: snat");
    return TC_ACT_OK;
}
