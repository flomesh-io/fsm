...
for pkg in "${pkgs[@]}"; do
   ...
   name=${pkg//\//_}-$date_s
   go test -benchmem -run=^$ -bench ^Benchmark github.com/flomesh-io/fsm/"$pkg" -cpuprofile "$bm_dir"/"$name".cpu.pprof -memprofile "$bm_dir"/"$name".mem.pprof -trace "$bm_dir"/"$name".trace >> "$output_file"
