...
var log = logger.New("fsm-preinstall")

func main() {
	log.Info().Msgf("Starting fsm-preinstall %s; %s; %s", version.Version, version.GitCommit, version.BuildDate)

	var verbosity string
	var enforceSingleMesh bool
	var namespace string

	flags := pflag.NewFlagSet("fsm-preinstall", pflag.ExitOnError)
...
func singleMeshOK(clientset kubernetes.Interface, enforceSingleMesh bool) func() error {
	return func() error {
		...
		if err != nil {
			return fmt.Errorf("listing FSM deployments: %w", err)
		}
...
func namespaceHasNoMesh(clientset kubernetes.Interface, namespace string) func() error {
	return func() error {
		...
		if err != nil {
			return fmt.Errorf("listing fsm-controller deployments in namespace %s: %w", namespace, err)
		}
