...
import (
	...
	fsmConfigClient "github.com/flomesh-io/fsm/pkg/gen/client/config/clientset/versioned"
...
fsm policy check-pods bookbuyer/bookbuyer-client bookstore/bookstore-server
...
fsm policy check-pods bookbuyer-client bookstore-server
`

const (
	namespaceSeparator       = "/"
	defaultFsmMeshConfigName = "fsm-mesh-config"
)
...
type trafficPolicyCheckCmd struct {
	...
	meshConfigClient fsmConfigClient.Interface
...
func (cmd *trafficPolicyCheckCmd) checkTrafficPolicy(srcPod, dstPod *corev1.Pod) error {
	...
	if permissiveMode, err := cmd.isPermissiveModeEnabled(); err != nil {
		...
		fmt.Fprintf(cmd.out, "[+] Permissive mode enabled for mesh operated by fsm-controller running in '%s' namespace\n\n "+
			...
			fsmNamespace, srcPod.Namespace, srcPod.Name, dstPod.Namespace, dstPod.Name)
		return nil
	}
