	...
	Describe("with pre-existing namespace", func() {
		...
		Context("adding namespace to the mesh that is ignored", func() {
			...
			BeforeEach(func() {
				...
				_, err = addDeployment(fakeClientSet, constants.FSMControllerName, testMeshName, "fsm-system-namespace", "testVersion0.1.2", true)
				Expect(err).To(BeNil())
		...
		Context("given one namespace as an arg without sidecar injection enabled", func() {
			...
			It("should correctly add a label to the namespace", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(ns.Labels[constants.FSMKubeResourceMonitorAnnotation]).To(Equal(testMeshName))
			})
		...
		Context("given two namespaces as args", func() {
			...
			It("should correctly add a label to all the namespaces", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(ns1.Labels[constants.FSMKubeResourceMonitorAnnotation]).To(Equal(testMeshName))

				ns2, err := fakeClientSet.CoreV1().Namespaces().Get(context.TODO(), testNamespace2, metav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				Expect(ns2.Labels[constants.FSMKubeResourceMonitorAnnotation]).To(Equal(testMeshName))
			})
		})

		Context("given one namespace with fsm-controller installed in it as an arg", func() {
			BeforeEach(func() {
				out = new(bytes.Buffer)
				fakeClientSet = fake.NewSimpleClientset()
				// mimic fsm controller deployment in testNamespace
...
var _ = Describe("Running the namespace remove command", func() {
	...
	Describe("with pre-existing namespace and correct label", func() {
		...
		It("should correctly remove the label on the namespace", func() {
			ns, err := fakeClientSet.CoreV1().Namespaces().Get(context.TODO(), testNamespace, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
			Expect(ns.Labels).ShouldNot(HaveKey(constants.FSMKubeResourceMonitorAnnotation))
		})
	})
		...
		It("should only have namespaces enlisted", func() {
			...
			Expect(namespaces.Items).To(gstruct.MatchAllElements(idSelector, gstruct.Elements{
				"enlisted1": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
					"ObjectMeta": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
						"Labels": gstruct.MatchKeys(gstruct.IgnoreMissing, gstruct.Keys{
							constants.FSMKubeResourceMonitorAnnotation: Equal("mesh1"),
						}),
					}),
				}),
				"enlisted2": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
					"ObjectMeta": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
						"Labels": gstruct.MatchKeys(gstruct.IgnoreMissing, gstruct.Keys{
							constants.FSMKubeResourceMonitorAnnotation: Equal("mesh2"),
						}),
					}),
				}),
			}))
...
func createNamespaceSpec(namespace, meshName string, enableSideCarInjection bool, ignoreNamespace bool, enableMetrics bool) *v1.Namespace {
	...
	if meshName != "" {
		labelMap[constants.FSMKubeResourceMonitorAnnotation] = meshName
	}
	ns := &v1.Namespace{
