	...
	testCases := []test{
		{
			...
			"fsm-namespace",
			"fsm-exception",
			"Error message with args such as [name: fsm-name], [namespace: fsm-namespace], and [err: fsm-exception]\n\n" + actionableMsg,
		},
	}
...
func TestAnnotateErrorMessageWithFsmNamespace(t *testing.T) {
	type test struct {
		errorMsg     string
		name         string
		namespace    string
	...
	for _, tc := range testCases {
		t.Run("Testing annotated error message", func(t *testing.T) {
			...
			assert.Equal(
				tc.annotatedMsg,
				annotateErrorMessageWithFsmNamespace(tc.errorMsg, tc.name, tc.namespace, tc.exceptionMsg).Error())
		})
	}
}
...
func addDeployment(fakeClientSet kubernetes.Interface, depName string, meshName string, namespace string, fsmVersion string, isMesh bool) (*v1.Deployment, error) {
	dep := createDeployment(depName, meshName, fsmVersion, isMesh)
...
func createDeployment(deploymentName, meshName string, fsmVersion string, isMesh bool) *v1.Deployment {
	labelMap := make(map[string]string)
	if isMesh {
		labelMap[constants.AppLabel] = constants.FSMControllerName
		labelMap["meshName"] = meshName
		labelMap[constants.FSMAppVersionLabelKey] = fsmVersion
	} else {
