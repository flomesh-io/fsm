...
func newNamespace(name string, annotations map[string]string) *corev1.Namespace {
	ns := &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name:   name,
			Labels: map[string]string{constants.FSMKubeResourceMonitorAnnotation: testMesh},
		},
	}
...
func TestIsMonitoredNamespace(t *testing.T) {
	...
	nsInvalidMonitorLabel := corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "ns-4",
			Labels: map[string]string{constants.FSMKubeResourceMonitorAnnotation: ""},
		},
	}
	...
	nsWrongMeshName := corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "ns-2",
			Labels: map[string]string{constants.FSMKubeResourceMonitorAnnotation: "some-mesh"},
		},
	}
