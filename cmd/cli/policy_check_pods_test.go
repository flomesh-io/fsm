...
func TestIsPermissiveModeEnabled(t *testing.T) {
	...
	fsmNamespace := &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name: "fsm-system",
		},
	}
	_, err := fakeK8sClient.CoreV1().Namespaces().Create(context.TODO(), fsmNamespace, metav1.CreateOptions{})
	assert.Nil(err)
	...
	testCases := []struct {
	...
	}{
		{
			configv1alpha2.MeshConfig{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: fsmNamespace.Name,
					Name:      defaultFsmMeshConfigName,
				},
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("Testing testcase %d", i), func(t *testing.T) {
			...
			_, err := fakeConfigClient.ConfigV1alpha2().MeshConfigs(fsmNamespace.Name).Create(context.TODO(), &tc.meshConfig, metav1.CreateOptions{})
			assert.Nil(err)
			...
			err = fakeConfigClient.ConfigV1alpha2().MeshConfigs(fsmNamespace.Name).Delete(context.TODO(), tc.meshConfig.Name, metav1.DeleteOptions{})
			assert.Nil(err)
		})
	}
}
...
func TestRun(t *testing.T) {
	...
	_, err = fakeConfigClient.ConfigV1alpha2().MeshConfigs(fsmNamespace.Name).Create(context.TODO(), meshConfig, metav1.CreateOptions{})
	assert.Nil(err)
