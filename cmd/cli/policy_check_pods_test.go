...
func TestIsPermissiveModeEnabled(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("Testing testcase %d", i), func(t *testing.T) {
			...
			_, err := fakeConfigClient.ConfigV1alpha3().MeshConfigs(fsmNamespace.Name).Create(context.TODO(), &tc.meshConfig, metav1.CreateOptions{})
			assert.Nil(err)
			...
			err = fakeConfigClient.ConfigV1alpha3().MeshConfigs(fsmNamespace.Name).Delete(context.TODO(), tc.meshConfig.Name, metav1.DeleteOptions{})
			assert.Nil(err)
		})
	}
}
...
func TestRun(t *testing.T) {
	...
	_, err = fakeConfigClient.ConfigV1alpha3().MeshConfigs(fsmNamespace.Name).Create(context.TODO(), meshConfig, metav1.CreateOptions{})
	assert.Nil(err)
