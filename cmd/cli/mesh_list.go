...
This command will list all the fsm control planes running in a Kubernetes cluster and controller pods.`
...
func newMeshList(out io.Writer) *cobra.Command {
	...
	f := cmd.Flags()
	f.Uint16VarP(&listCmd.localPort, "local-port", "p", constants.FSMHTTPServerPort, "Local port to use for port forwarding")
...
func (l *meshListCmd) run() error {
	...
	if len(meshInfoList) == 0 {
		fmt.Fprintf(l.out, "No fsm mesh control planes found\n")
		return nil
	}
	...
	fmt.Fprintf(l.out, "\nTo list the FSM controller pods for a mesh, please run the following command passing in the mesh's namespace\n")
	fmt.Fprintf(l.out, "\tkubectl get pods -n <fsm-mesh-namespace> -l app=fsm-controller\n")
