...
func (c *MemoryCache) Get(key string) (*dns.Msg, bool, error) {
	...
	if ok && mesg.Msg == nil {
		ok = false
		log.Warn().Msgf("Cache: key %s returned nil entry", key)
		c.removeNoLock(key)
	}
	if ok {
		...
		for _, answer := range mesg.Msg.Answer {
			if elapsed > answer.Header().Ttl {
				log.Debug().Msgf("Cache: Key expired %s", key)
				c.removeNoLock(key)
				expired = true
			}
	...
	if !ok {
		log.Debug().Msgf("Cache: Cannot find key %s\n", key)
...
func (c *MemoryCache) Set(key string, msg *dns.Msg, blocked bool) error {
	...
	if msg == nil {
		log.Debug().Msgf("Setting an empty value for key %s", key)
	}
	c.mu.Lock()
...
func (c *MemoryBlockCache) Set(key string, value bool) error {
	...
	if strings.HasPrefix(key, "~") {
		...
		if err != nil {
			log.Error().Msgf("Invalid regexp entry: `%s` %v", key, err)
		} else {
			c.Special[key] = re
		}
	} else if strings.ContainsAny(key, globChars) {
