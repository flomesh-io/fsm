...
func TestUpdateMetric(t *testing.T) {
	...
	namespace := corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				constants.FSMKubeResourceMonitorAnnotation: "fsm",
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			expectedResp := fmt.Sprintf(`# HELP fsm_resource_namespace_count Represents the number of monitored namespaces in the service mesh
# TYPE fsm_resource_namespace_count gauge
fsm_resource_namespace_count %s
...
func TestQueueLenMetric(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		return metricsstore.DefaultMetricsStore.Contains(`fsm_events_queued ` + strconv.Itoa(numEvents) + "\n")
