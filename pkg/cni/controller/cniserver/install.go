...
const (
	fsmCniName         = "fsm-cni"
	kubeConfigFileName = "ZZZ-fsm-cni-kubeconfig"
...
contexts:
- name: fsm-cni-context
  context:
    cluster: local
    user: fsm-cni
current-context: fsm-cni-context
`
)
...
func (in *installer) Cleanup() error {
	...
	cniBinPath := path.Join(config.CNIBinDir, fsmCniName)
...
func copyBinaries() error {
	...
	err := util.AtomicCopy(srcFile, config.CNIBinDir, fsmCniName)
	if err != nil {
		return err
	}
