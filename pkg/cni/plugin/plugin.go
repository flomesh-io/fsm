...
func CmdAdd(args *skel.CmdArgs) error {
	...
	if err != nil {
		log.Error().Msgf("fsm-cni cmdAdd failed to parse config %v %v", string(args.StdinData), err)
	} else {
		k8sArgs := K8sArgs{}
		if err = types.LoadArgs(args.Args, &k8sArgs); err != nil {
			log.Error().Msgf("fsm-cni cmdAdd failed to load args %v %v", string(args.StdinData), err)
		} else {
