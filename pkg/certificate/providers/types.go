...
const (
	// TresorKind represents Tresor, an internal package which leverages Kubernetes secrets and signs certs on the FSM pod
	TresorKind Kind = "tresor"

	// VaultKind represents Hashi Vault; FSM is pointed to an external Vault; signing of certs happens on Vault
...
type VaultOptions struct {
	...
	VaultHost                 string
	VaultToken                string // TODO(#4745): Remove after deprecating the fsm.vault.token option. Replace with VaultTokenSecretName
	VaultRole                 string
	VaultPort                 int
