...
// LoadBalancerPolicyInformer provides access to a shared informer and lister for
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.GatewayV1alpha1().LoadBalancerPolicies(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.GatewayV1alpha1().LoadBalancerPolicies(namespace).Watch(context.TODO(), options)
			},
		},
		&policyattachmentv1alpha1.LoadBalancerPolicy{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *loadBalancerPolicyInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&policyattachmentv1alpha1.LoadBalancerPolicy{}, f.defaultInformer)
}

func (f *loadBalancerPolicyInformer) Lister() v1alpha1.LoadBalancerPolicyLister {
	return v1alpha1.NewLoadBalancerPolicyLister(f.Informer().GetIndexer())
}
