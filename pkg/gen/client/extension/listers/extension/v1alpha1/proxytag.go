...
func NewProxyTagLister(indexer cache.Indexer) ProxyTagLister {
	return &proxyTagLister{indexer: indexer}
}

// List lists all ProxyTags in the indexer.
func (s *proxyTagLister) List(selector labels.Selector) (ret []*v1alpha1.ProxyTag, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ProxyTag))
	})
	return ret, err
}
...
func (s *proxyTagLister) ProxyTags(namespace string) ProxyTagNamespaceLister {
	return proxyTagNamespaceLister{indexer: s.indexer, namespace: namespace}
}
...
// List lists all ProxyTags in the indexer for a given namespace.
func (s proxyTagNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ProxyTag, err error) {
...
// Get retrieves the ProxyTag from the indexer for a given namespace and name.
func (s proxyTagNamespaceLister) Get(name string) (*v1alpha1.ProxyTag, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("proxytag"), name)
	}
	return obj.(*v1alpha1.ProxyTag), nil
}
