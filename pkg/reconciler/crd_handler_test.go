...
func TestCRDEventHandlerUpdateFunc(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			updatedCrd: apiv1.CustomResourceDefinition{
				...
				ObjectMeta: metav1.ObjectMeta{
					Name: "meshconfigs.config.flomesh.io.NEW",
					Labels: map[string]string{
						constants.FSMAppNameLabelKey: constants.FSMAppNameLabelValue,
						constants.ReconcileLabel:     strconv.FormatBool(true),
					},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if expectedMetric > 0 {
				a.True(metricsstore.DefaultMetricsStore.Contains(`fsm_reconciliation_total{kind="CustomResourceDefinition"} ` + strconv.Itoa(expectedMetric) + "\n"))
			}
		})
	}
}
...
func TestIsLabelModified(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			key:           constants.FSMAppNameLabelKey,
			value:         constants.FSMAppNameLabelValue,
			labelModified: false,
		},
		{
