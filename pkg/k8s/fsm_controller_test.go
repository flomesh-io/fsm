...
func TestGetFSMControllerPods(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName: "multiple pods (fsm-controller pods and other pods) in multiple namespaces",
			pods: []*corev1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "fsm-controller-pod-1",
				...
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "some-other-fsm-controller-pod",
			...
			expectedPodNames: []string{
				"fsm-controller-pod-1",
				"fsm-controller-pod-2",
			},
		},
		{
			testName:         "no pods",
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			...
			podList := GetFSMControllerPods(fakeClientSet, testNamespace)
