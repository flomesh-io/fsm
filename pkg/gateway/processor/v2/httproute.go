...
func (c *ConfigGenerator) toV2HTTPRouteRule(httpRoute *gwv1.HTTPRoute, rule *gwv1.HTTPRouteRule, ruleIndex int, holder status.RouteParentStatusObject) *fgwv2.HTTPRouteRule {
	...
	if len(rule.Filters) > 0 {
		r2.Filters = c.toV2HTTPRouteFilters(httpRoute, fmt.Sprintf("%d", ruleIndex), rule.Filters, holder)
	}
...
func (c *ConfigGenerator) toV2HTTPBackendRefs(httpRoute *gwv1.HTTPRoute, rule *gwv1.HTTPRouteRule, ruleIndex int, holder status.RouteParentStatusObject) []fgwv2.HTTPBackendRef {
	...
	for i, bk := range rule.BackendRefs {
		if svcPort := c.backendRefToServicePortName(httpRoute, bk.BackendRef.BackendObjectReference, holder); svcPort != nil {
			...
			if len(bk.Filters) > 0 {
				b2.Filters = c.toV2HTTPRouteFilters(httpRoute, fmt.Sprintf("%d-%d", ruleIndex, i), bk.Filters, holder)
			}
...
func (c *ConfigGenerator) toV2HTTPRouteFilters(httpRoute *gwv1.HTTPRoute, index string, routeFilters []gwv1.HTTPRouteFilter, holder status.RouteParentStatusObject) []fgwv2.HTTPRouteFilter {
