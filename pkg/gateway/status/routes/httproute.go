			...
			if !found {
				if svcPort.AppProtocol != nil &&
					*svcPort.AppProtocol == constants.AppProtocolWSS &&
					...
					defer p.recorder.Eventf(route, corev1.EventTypeWarning, string(gwv1.RouteReasonUnsupportedProtocol), "No matching BackendTLSPolicy was found for the backend protocol %q and appProtocol %q", svcPort.Protocol, *svcPort.AppProtocol)
