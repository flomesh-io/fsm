...
import (
	...
	fakePolicyClientset "github.com/flomesh-io/fsm/pkg/gen/client/policy/clientset/versioned/fake"
...
func TestIngressBackendValidator(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			informerCollection, err := informers.NewInformerCollection("fsm", stop, informers.WithPolicyClient(fakeClient))
			assert.NoError(err)
...
func TestEgressValidator(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expResp:   nil,
			expErrStr: "Expected 'matches.apiGroup' to be one of [specs.smi-spec.io/v1alpha4 policy.flomesh.io/v1alpha1], got: invalid",
		},
		{
				...
				Object: runtime.RawExtension{
					Raw: []byte(`
					{
						...
						"spec": {
							"matches": [
								{
								"apiGroup": "policy.flomesh.io/v1alpha1",
...
func TestUpstreamTrafficSettingValidator(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expResp:   nil,
			expErrStr: "invalid responseStatusCode 1",
		},
		{
