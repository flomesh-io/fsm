...
func NewValidatingWebhook(ctx context.Context, cfg *configurator.Client, webhookConfigName, fsmNamespace, fsmVersion, meshName string, enableReconciler, validateTrafficTarget bool, certManager *certificate.Manager, kubeClient kubernetes.Interface, kubeController k8s.Controller, policyClient policy.Controller) error {
	...
	srv, err := webhook.NewServer(ValidatorWebhookSvc, fsmNamespace, constants.ValidatorWebhookPort, certManager, map[string]http.HandlerFunc{
		validationAPIPath: v.doValidation,
	}, func(cert *certificate.Certificate) error {
		if err := createOrUpdateValidatingWebhook(kubeClient, cert, webhookConfigName, meshName, fsmNamespace, fsmVersion, validateTrafficTarget, enableReconciler); err != nil {
			return err
		}
		return nil
	})
