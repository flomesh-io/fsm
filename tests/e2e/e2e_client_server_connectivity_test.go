...
var _ = FSMDescribe("Test HTTP traffic from 1 pod client -> 1 pod server",
	FSMDescribeInfo{
		Tier:   1,
		Bucket: 7,
		OS:     OSCrossPlatform,
...
func testTraffic(withSourceKubernetesService bool, destPodCommand []string, destServiceHeadless bool, installOpts ...InstallFsmOpt) {
	...
	It("Tests HTTP traffic for client pod -> server pod", func() {
		// Install FSM
		Expect(Td.InstallFSM(Td.GetFSMInstallOpts(installOpts...))).To(Succeed())
