...
var _ = FSMDescribe("Test HTTP traffic from 1 pod client -> 1 pod server before and after fsm-controller restart",
	FSMDescribeInfo{
		Tier:   2,
		Bucket: 1,
		OS:     OSCrossPlatform,
	...
	func() {
		Context("SimpleClientServer traffic test involving fsm-controller restart: HTTP", func() {
...
func testHTTPTrafficWithControllerRestart() {
	...
	It("Tests HTTP traffic for client pod -> server pod", func() {
		...
		By("Restarting FSM controller")
		Expect(Td.RestartFSMController(Td.GetFSMInstallOpts())).To(Succeed())
		...
		Expect(cond).To(BeTrue(), "Failed testing HTTP traffic from source pod %s to destination service %s after fsm-controller restart", srcPod.Name, dstSvc.Name)
	})
}
