...
- `Rate limiting`: Added capability to perform local per-instance [rate limiting of TCP connections and HTTP requests](https://release-v1-2.docs.flomesh.io/docs/guides/traffic_management/rate_limiting).
...
- The `fsm_proxy_response_send_success_count` and `fsm_proxy_response_send_error_count` metrics are now labeled with the proxy certificate's common name and XDS type, so queries to match the previous equivalent need to sum for all values of each of those labels.

### Deprecation notes

The following capabilities have been deprecated and cannot be used.

- The `fsm_injector_injector_sidecar_count` and `fsm_injector_injector_rq_time` metrics have been removed. The `fsm_admission_webhook_response_total` and `fsm_http_response_duration` metrics should be used instead.
- FSM will no longer support installation on Kubernetes version v1.19.
...
- Top level Helm chart keys are renamed from `FlomeshServiceMesh` to `fsm`
- `fsm mesh upgrade` no longer carries over values from previous releases. Use the `--set` flag on `fsm mesh upgrade` to pass values as needed. The `--container-registry` and `--fsm-image-tag` flags have also been removed in favor of `--set`.
